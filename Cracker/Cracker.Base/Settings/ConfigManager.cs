...
namespace Cracker.Base.Settings
{
	public class ConfigManager
	{
		...
		{
			this.configPatn = Path.Combine(currentDirectory, "appsettings.json"); ;
		}
		public OperationResult<Config> Build()
		{
			...
			if (string.IsNullOrEmpty(config.HashCatPath))
			{
				Log.Message($"{configPatn} не содержит секцию {nameof(config.HashCatPath)}");
				allGood = false;
			}

			if (!File.Exists(config.HashCatPath))
			{
				Log.Message($"Hashcat не найден по пути {config.HashCatPath}");
				allGood = false;
			}
